                 ...
                 END IF;
                    ...
                    ELSE
                        IF v_id_moneda_base = p_id_moneda[v_cont] THEN
                        	...
                        	select ('('||tp.codigo||') - '||tp.nombre_partida)::varchar
                            into v_partida
                            from pre.tpartida tp
                            where tp.id_partida = p_id_partida[v_cont];

                            select ('('||ttc.codigo||') - '||ttc.descripcion)::varchar
                            into v_presupuesto
                            from param.tcentro_costo tcc
        					inner join param.ttipo_cc ttc on ttc.id_tipo_cc = tcc.id_tipo_cc
                            where tcc.id_centro_costo = p_id_presupuesto[v_cont];

                            raise exception 'solo se tiene disponible un monto en moneda base de:  %, en el presupuesto: % para la partida: %', v_resultado_ges[3], v_presupuesto, v_partida;
                        ELSE
