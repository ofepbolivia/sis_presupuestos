...
/***********************************I-DEP-BVP-PRE-0-30/08/2019*****************************************/
CREATE VIEW pre.vpresupuesto_cc_x_partida (
    id_centro_costo,
    estado_reg,
    id_ep,
    id_gestion,
...
SELECT cec.id_centro_costo,
    ...
    pre.id_proceso_wf,
    tp.movimiento AS movimiento_tipo_pres,
    ((('('::text || tp.codigo::text) || ') '::text) ||
        tp.nombre::text)::character varying AS desc_tipo_presupuesto,
    tp.sw_oficial,
    pre.sw_consolidado,
    pre.id_categoria_prog,
        CASE
            WHEN cec.id_tipo_cc IS NOT NULL THEN ((('('::text ||
                cec.codigo_tcc::text) || ') '::text) || cec.descripcion_tcc::text)::character varying
            ELSE pre.descripcion
        END AS descripcion,
    cp.id_cp_programa,
    cp.id_cp_actividad,
    cp.id_cp_fuente_fin,
    cp.id_cp_organismo_fin,
    cp.id_cp_proyecto,
    cp.id_unidad_ejecutora
FROM param.vcentro_costo cec
     JOIN segu.tusuario usu1 ON usu1.id_usuario = cec.id_usuario_reg
     LEFT JOIN segu.tusuario usu2 ON usu2.id_usuario = cec.id_usuario_mod
     JOIN param.vep ep ON ep.id_ep = cec.id_ep
